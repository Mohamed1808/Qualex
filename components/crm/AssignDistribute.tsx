...
export default function AssignDistribute() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      <h1 className="text-xl font-semibold text-[#111827]">Assign & Distribute</h1>

      <div className="flex gap-1 border-b border-[#e5e7eb]">
        {([['assign', 'Manual Assign'], ['move', 'Move Lead'], ['schedule', 'Scheduled Distribution']] as const).map(([k, label]) => (
          <button key={k} onClick={() => setTab(k)}
            className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === k ? 'text-[#5757e6] border-[#5757e6]' : 'text-[#6B7280] border-transparent hover:text-[#111827]'}`}>
            {label}
          </button>
        ))}
      </div>
            ...
            {agents.map((a) => {
              ...
              return (
                <button key={a.id} onClick={() => setTargets((t) => on ? t.filter((x) => x !== a.id) : [...t, a.id])}
                  className={`text-xs px-3 py-1.5 rounded-lg border ${on ? 'border-[#5757e6] bg-[#5757e6]/15 text-[#4444cc]' : 'border-[#e5e7eb] text-[#4B5563] hover:border-[#d1d5db]'}`}>
                  {a.full_name}
                </button>
              )
            })}
