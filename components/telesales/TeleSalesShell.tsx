...
const navItems = [
  { href: '/telesales/agent', label: 'My Queue', icon: '📋', roles: ['telesales_agent'] },
  {
    href: '/telesales/supervisor',
    label: 'Live Queue',
    icon: '👁️',
  ...
  {
    href: '/telesales/supervisor/duplicates',
    label: 'Duplicates',
    icon: '🔁',
    roles: ['telesales_supervisor'],
  },
...
export default function TeleSalesShell({ profile, initialAttendance, children }: TeleSalesShellProps) {
  ...
  const { attendance, checkIn, startBreak, endBreak, checkOut } = useAttendance(profile.id)
  ...
  return (
    <div className="flex h-screen bg-[#0f0f0f] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* On Break Banner */}
        {isOnBreak && (
          <div className="bg-[#F59E0B] text-black text-xs font-semibold text-center py-1.5 px-4 flex items-center justify-center gap-2 flex-shrink-0">
            <span>☕</span>
            <span>You are currently on break. Return to resume taking leads.</span>
          </div>
        )}

        {/* Checked Out Banner */}
        {isCheckedOut && !isOnBreak && (
          <div className="bg-[#F26161] text-white text-xs font-semibold text-center py-1.5 px-4 flex items-center justify-center gap-2 flex-shrink-0">
            <span>🔴</span>
            <span>You have checked out. No new leads will be assigned until you check in again.</span>
          </div>
        )}
