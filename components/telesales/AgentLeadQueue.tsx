...
export default function AgentLeadQueue({ userId, role, initialLeads }: AgentLeadQueueProps) {
  const router = useRouter()
  const { data: leads, isLoading } = useLeadQueue({ userId, role, initialData: initialLeads })

  const sortedLeads = [...(leads ?? [])].sort((a, b) => {
    // SLA breached first, then by SLA due date ascending
    if (a.tele_sla_breached && !b.tele_sla_breached) return -1
    if (!a.tele_sla_breached && b.tele_sla_breached) return 1
    if (a.tele_sla_due_at && b.tele_sla_due_at) {
      return new Date(a.tele_sla_due_at).getTime() - new Date(b.tele_sla_due_at).getTime()
    }
    return 0
  })
    ...
    <div className="p-6">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm text-[#6B7280]">Great work! Check back soon for new assignments.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
