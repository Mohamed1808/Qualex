...
export default async function TSAttendancePage() {
  if (isPreviewMode()) {
    const agents = MOCK_PROFILES.filter((p) => p.role === 'telesales_agent')
    ...
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-xl font-semibold text-white">Attendance Board</h1>
          <p className="text-sm text-[#6B7280] mt-0.5">
            {format(toZonedTime(new Date(), CAIRO_TZ), 'EEEE, MMMM d, yyyy')} — Cairo Time
          </p>
        </div>
              ...
              {agents.map((agent) => {
                ...
                return (
                  <tr key={agent.id} className="border-b border-[#2a2a2a] last:border-0">
                    <td className="px-4 py-3"><p className="text-white font-medium">{agent.full_name}</p></td>
  ...
  const { data: attendance } = await supabase
    ...
    .select(`
      *,
      profiles:agent_id(id, full_name, role, is_on_break)
    `)
    .eq('date', today)
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-[#9CA3AF] text-xs">
                    {att?.checked_in_at
                      ? format(
                          toZonedTime(new Date(att.checked_in_at), CAIRO_TZ),
                          'h:mm a'
                        )
                      : '—'}
                  </td>
                  ...
                  <td className="px-4 py-3 text-[#9CA3AF] text-xs">
                    {att?.checked_out_at
                      ? format(
                          toZonedTime(new Date(att.checked_out_at), CAIRO_TZ),
                          'h:mm a'
                        )
                      : '—'}
                  </td>
